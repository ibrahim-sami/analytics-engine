WITH current_week_data AS (
  ...
  COUNT(project_id) as num_workflows,
  COUNT(user_employee_code) AS num_agents,
  SUM(total_submission_count) AS num_submissions,
  SUM(total_submission_count)/AVG(avg_submission_duration_minutes) AS num_submissions_per_min,
...
previous_week_data AS (
  SELECT project_group_id, project_name, 
  COUNT(project_id) as num_workflows_prev,
  COUNT(user_employee_code) AS num_agents_prev,
  SUM(total_submission_count) AS num_submissions_prev,
  SUM(total_submission_count)/AVG(avg_submission_duration_minutes) AS num_submissions_per_min_prev,
...
a.num_submissions_per_min, b.num_submissions_per_min_prev,
